...
def stateToTrajTensors(trunc_states):
...
class ValueNetwork(nn.Module):
    ...
    def forward(self, states):
        ...
        num_humans = rel_trajs.shape[-2]-1 # Not flexible for different amounts of human
...
class TLSGAN(MultiHumanRL):
    ...
    def predict(self, states):
        ...
        if self.policy_learning:
            ...
            next_position = self.model((trajs, rel_trajs, self_info)).data.squeeze(0)
            ...
            if self.phase == 'train' and probability < self.epsilon:
                ...
                for action in self.action_space:
                    ...
                    # VALUE UPDATE
                    trajs, rel_trajs, self_info = stateToTrajTensors([states + [JointState(next_self_state, next_human_states)]])
                    next_state_value = self.model((trajs, rel_trajs, self_info)).data.item()
